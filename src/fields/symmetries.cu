...
double Fields::CheckDivergence(  ){
    ...
    if (param_ptr->incompressible) {
        data_type* complex_Vel = d_all_fields + grid.NTOTAL_COMPLEX * vars.VEL;

        Divergence<<<blocksPerGrid, threadsPerBlock>>>(wavevector.d_all_kvec, complex_Vel, (data_type *)  d_tmparray[0], (size_t) grid.NTOTAL_COMPLEX);
        ...
        meanFieldDiv = FieldDiv/(2 * grid.NTOTAL_COMPLEX);
        std::printf("---- Mean-divergence of v-field is %.2e [< div Field> * Delta x]\n", meanFieldDiv*deltax);

    }
    if (param_ptr->mhd) {

        data_type* complex_Mag = d_all_fields + grid.NTOTAL_COMPLEX * vars.MAG;

        Divergence<<<blocksPerGrid, threadsPerBlock>>>(wavevector.d_all_kvec, complex_Mag, (data_type *)  d_tmparray[0], (size_t) grid.NTOTAL_COMPLEX);
        // transform back to real space
        c2r_fft(d_tmparray[0], d_tmparray_r[0]);
        // compute absolute value of real vector (actually Dasum already does it...)
        // reduce sum
