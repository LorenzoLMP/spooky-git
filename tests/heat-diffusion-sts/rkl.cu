...
RKLegendre::RKLegendre(Parameters &p_in, Supervisor &sup_in) {
    ...
    else if (!sts_algorithm.compare(std::string("rkl1"))) {
        // sts_algorithm = "rkl2";
        std::printf("num rkl scratch arrays: %d \n",4*num_sts_vars);
    ...
    else if (!sts_algorithm.compare(std::string("rkl2"))) {
        // sts_algorithm = "rkl2";
        std::printf("num rkl scratch arrays: %d \n",4*num_sts_vars);
...
void RKLegendre::compute_cycle(data_type* complex_Fields, scalar_type* real_Buffer){
    ...
    else if (!param_ptr->sts_algorithm.compare(std::string("rkl1"))) {
        compute_cycle_RKL1(complex_Fields, real_Buffer);
    }
    else if (!param_ptr->sts_algorithm.compare(std::string("rkl2"))) {
        compute_cycle_RKL2(complex_Fields, real_Buffer);
    }
...
void RKLegendre::compute_cycle_RKL1(data_type* complex_Fields, scalar_type* real_Buffer){
    ...
    // static double **v;
    double s_str;                          /* The "stage" parameter */

    // int i;
    int nv, s_RKL = 0;
    double scrh;
    // int nv_indx, var_list[vars.NUM_FIELDS], nvar_rkl;
    double mu_j, nu_j, mu_tilde_j;
    // data_type Y;
    // double a_jm1, b_j, b_jm1, b_jm2, 
    double w1;
    int stage = 1;
    ...
    for (nv = 0; nv < num_sts_vars; nv++){
        ...
        // dU0 <- dU
        ComplexVecAssign<<<blocksPerGrid, threadsPerBlock>>>(d_farray_dU[nv], d_farray_dU0[nv], grid.NTOTAL_COMPLEX);
    ...
    for (stage = 2; stage <= s_RKL; stage++) {
        ...
        mu_j       = (2.*stage -1.)/stage;   
        mu_tilde_j = w1*mu_j;
        nu_j       = -(stage -1.)/stage;


        if (param_ptr->debug > 0) {
            std::printf("STS::::: RKL subcycle num: %d \n", stage);
        
            std::printf("mu_j: %.4f, mu_tilde_j: %.4f, nu_j: %.4f \n", mu_j, mu_tilde_j, nu_j);
        }
...
void RKLegendre::compute_cycle_RKL2(data_type* complex_Fields, scalar_type* real_Buffer){
    ...
    /*  Solution of quadratic Eq. 4*tau/dt_exp = stage^2 + stage - 2  */
    // s_RKL = int(0.5*(sqrt(9.0 + 16.0*scrh) - 1) + 1); 
    // if (s_RKL % 2 == 0){
        // s_RKL -= 1;
    // }
    ...
    for (stage = 2; stage <= s_RKL; stage++) {
        ...
        mu_j       = (2.*stage -1.)/stage * b_j/b_jm1;   /* Eq. [17] */
        mu_tilde_j = w1*mu_j;
        gamma_j    = -a_jm1*mu_tilde_j;
        nu_j       = -(stage -1.)*b_j/(stage*b_jm2);

        if (param_ptr->debug > 0) {
            std::printf("STS::::: RKL subcycle num: %d \n", stage);
        
            std::printf("mu_j: %.4f, mu_tilde_j: %.4f, gamma_j: %.4f, nu_j: %.4f \n", mu_j, mu_tilde_j, gamma_j, nu_j);
            std::printf("b_jm2: %.4f, b_jm1: %.4f, a_jm1: %.4f, b_j: %.4f \n", b_jm2, b_jm1, a_jm1, b_j);
        }
        ...
        b_jm1 = b_j;
        a_jm1 = 1.0 - b_jm1;
        b_j   = 0.5*(stage*stage+3.0*stage)/(stage*stage+3.0*stage+2); // from before
        // b_j   = 0.5*(stage*stage + stage - 2)/(stage*stage+stage);  // eq 16 Meyer 2012
