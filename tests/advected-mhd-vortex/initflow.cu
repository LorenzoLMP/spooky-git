...
void Fields::init_SpatialStructure(Parameters *param){
 ...
 //    // Initialize the arrays
	...
	for (int i = 0; i < 2*ntotal_complex; i++){
		...
		farray_r[VX][i] = 0.1 - 1./(2.0 * M_PI) * exp( (1.0  - 100.0*R2)/2.0 ) * y[i] ;
		farray_r[VY][i] = 0.1 + 1./(2.0 * M_PI) * exp( (1.0  - 100.0*R2)/2.0 ) * x[i] ;
		farray_r[VZ][i] = 0.0 ;

		farray_r[BX][i] = - 1./(2.0 * M_PI) * exp( (1.0  - 100.0*R2)/2.0 ) * y[i] ;
		farray_r[BY][i] =   1./(2.0 * M_PI) * exp( (1.0  - 100.0*R2)/2.0 ) * x[i] ;
		farray_r[BZ][i] = 0.0 ;
